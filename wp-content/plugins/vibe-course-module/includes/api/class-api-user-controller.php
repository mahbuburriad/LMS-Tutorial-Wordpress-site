		...
		function add_to_course($request){
			...
			if(is_numeric($post->course_id)){
				if(get_post_type($post->course_id) == 'course'){
					...
					$data = array('status'=>true, 'message'=>_x('Successfully  subscribed to course!','course subscribe via app','vibe','status'=>bp_course_get_user_course_status($this->user_id,$post->course_id),'expiry'=>bp_course_get_user_expiry_time($this->user_id,$post->course_id)));		
				}
			}else{
		...
		function renew_course($request){
			...
			if(is_numeric($post->course_id) && is_numeric($post->duration)){
				if(get_post_type($post->course_id) == 'course'){
					...
					$data = array('status'=>true, 'message'=>_x('Successfully renewed','vibe','status'=>bp_course_get_user_course_status($this->user_id,$post->course_id),'expiry'=>bp_course_get_user_expiry_time($this->user_id,$post->course_id)));		
				}
			}else{
				$data = array('status'=>false, 'message'=>_x('Failed to subscribe to course','course subscribe via app','vibe'));
			}
