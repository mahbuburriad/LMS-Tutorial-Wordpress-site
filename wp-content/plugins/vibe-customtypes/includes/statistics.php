...
function lms_stats_administrator_report($subtab='overview'){
	...
	$total_courses_published=$wpdb->get_var("
...
?>
	...
	<div id="poststuff" class="vibe-reports-wrap">
		<div class="vibe-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Courses in Total:', 'vibe-customtypes' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_courses_published > 0 ) echo $total_courses_published; else _e( 'n/a', 'vibe-customtypes' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<div class="inside">
				    ...
				    var data = google.visualization.arrayToDataTable([
				     ...
				     foreach ($course_title as $course) {
				    		...
				    		$student_count = count(bp_course_get_students_undertaking($course->ID,999));
				            echo "['".$title."', ".$student_count."],";
				      }
				      ?>
				    ]);
				    
				    var options = {
				        title: 'Courses with Student Enrolled',
				        width: 250,
				        height: 250,
				    };
		...
		<div class="vibe-reports-main">
				...
				<h2>Top Courses By Learners in Progress</h2>
					<h3><label>Course Title</label><label># Students Enrolled</label></h3>
					<div class="inside">
						<ul>
						<?php
							foreach ($course_title as $course) {
							echo '<li><label>'.$course->post_title.'</label>
							<label>'.count(bp_course_get_students_undertaking($course->ID,999)).'</label></li>';
							}
						?>
						</ul>
					</div>
			...
			<div class="vibe-reports-main">
				<div class="postbox course_info">
					<h2>Most Recent Completions</h2>
					<h3><label>Students</label><label>Courses</label><label>Completion Date</label></h3>
					<div class="inside"><ul>
						<?php
						$student_info=array();
						foreach($stats as $st){
						...
						if(isset($status)){
						$student_info['user'] = get_the_author_meta('display_name',$st->user);
						$student_info['course'] = (isset($st->course)?$st->course:'n/a');
						echo '<li><label>'.$student_info['user'].'</label><label>'.$student_info['course'].'</label></li>';
						}
						}
