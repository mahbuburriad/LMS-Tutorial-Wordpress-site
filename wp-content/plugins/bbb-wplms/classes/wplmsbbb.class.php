...
{   
    class Wplms_Bbb  // We'll use this just to avoid function name conflicts 
    {
        ...
        function wplms_bbb_content() {
            ...
            echo '<thead><tr><th>'._x('Meeting name','','vibe').'</th><th>'._x('Privacy','','vibe').'</th><th>'._x('Status','','vibe').'</th><th>'._x('Action','','vibe').'</th></tr></thead><tbody>';
        ...
        function reset_crons_on_edit($old_options_data,$option_data,$meeting_id){
            ...
            if(!empty($option_data['start_date'])){
                ...
                if(  $timestamp  > $utc_time){
                    ...
                    if(!empty($users)){
                        $args =  array($options_data['name'], $users);
                        if(!wp_next_scheduled('wplms_send_wplms_bbb_reminders',$args))
                            wp_schedule_single_event($timestamp,'wplms_send_wplms_bbb_reminders',$args);
                    }
                }
        ...
        function create_new_meeting(){
            ...
            if(!empty($data) && function_exists('bigbluebutton_generateToken') && function_exists('bigbluebutton_generatePasswd')){
                ...
                $meetingName = htmlspecialchars(stripcslashes($data[ 'meetingName' ]));
                $attendeePW = $data[ 'attendeePW' ]? $data[ 'attendeePW' ]: bigbluebutton_generatePasswd(6, 2);
                $moderatorPW = $data[ 'moderatorPW' ]? $data[ 'moderatorPW' ]: bigbluebutton_generatePasswd(6, 2, $attendeePW);
                $waitForModerator = (isset($data[ 'waitForModerator' ]) &&  $data[ 'waitForModerator' ] == 'True')? true: false;

                $recorded = (isset( $data[ 'recorded' ]) &&  $data[ 'recorded' ] == 'True')? true: false;
                $meetingVersion = time();
                ...
                if($meetingName == '' || $attendeePW == '' || $moderatorPW == '') {
                    ...
                    if(!$alreadyExists) {
                        ...
                        if(!empty($meetingID) && !empty($meetingName)){
                            ...
                            if(!empty($option_data) && is_array($option_data)){
                                $existing[$meetingID] = $option_data;
            ...
            if($flag || current_user_can('manage_options')){
                ...
                if(!empty($meeting['id']) && !empty($data['meetingName'])){
                    ...
                    if(!empty($option_data) && is_array($option_data)){
                        ...
                        $attendeePW = '';
                        $table= $wpdb->prefix.'bigbluebutton';
                        if(!empty($data['attendeePW'])){
                            $attendeePW = $data['attendeePW'];
                        }
                        if(!empty($data['moderatorPW'])){
                            $moderatorPW = $data['moderatorPW'];
                        }
                        if(!empty($data['waitForModerator']) &&  $data[ 'waitForModerator' ] == 'True'){
                            $waitForModerator = 1;
                        }
                        if(!empty($data['recorded']) &&  $data[ 'recorded' ] == 'True'){
                            $recorded = 1;
                        }
                        ...
                        $wpdb->update( 
                            $table, 
                            array( 
                                'meetingName' => $data['meetingName'],  // string
                                'attendeePW' =>$attendeePW,   // integer (number) 
                ...
                if(!$response || $response['returncode'] == 'FAILED' ) {//If the server is unreachable, or an error occured
                    ...
                    if ( (BigBlueButton::isMeetingRunning( $found->meetingID, $url_val, $salt_val ) && ($found->moderatorPW == $password || $found->attendeePW == $password ) )
                        ...
                        if(empty($popup)){
                            ...
                            $out .= '<a class="wplms-bbb-meeting-popup button" href="'.$bigbluebutton_joinURL.'">'._x("Open Meeting").'</a>';
                    ...
                    //for the moderator to start the meeting
                        ...
                        $out .= $this->wplms_bigbluebutton_display_redirect_script($meeting,$bigbluebutton_joinURL, $found->meetingID, $found->meetingName, $name,$popup);
                        return $out;
                    }
                }
            }
        ...
        function wplms_bigbluebutton_display_redirect_script($meeting,$bigbluebutton_joinURL, $meetingID, $meetingName, $name,$popup=null) {
            if(empty($popup)){
                ...
                $embed  .= '<a class="wplms-bbb-meeting-popup button" href="'.$bigbluebutton_joinURL.'">'._x("Open Meeting").'</a>';
            }
            $out .= '
        ...
        function print_edit(){
            ...
            if(!empty($_GET['meeting'])){
                ...
                if(!is_numeric($_GET['meeting'])){
                    $sql = "SELECT * from {$table} WHERE meetingID = '".$_GET['meeting']."'";
                    $meeting_post = $wpdb->get_row($sql);
