...
{	
    ...
    add_action('init',function(){
    	...
    	if ( in_array( 'vibe-customtypes/vibe-customtypes.php', apply_filters( 'active_plugins', $active_plugins ) ) && in_array( 'vibe-course-module/loader.php', apply_filters( 'active_plugins', $active_plugins) ) && in_array( 'bigbluebutton/bigbluebutton-plugin.php', apply_filters( 'active_plugins',$active_plugins ) )  && in_array( 'buddypress/bp-loader.php', apply_filters( 'active_plugins',$active_plugins ) )) {
    			$Wplms_Bbb = Wplms_Bbb::init();
    	}
    	if(!in_array( 'bigbluebutton/bigbluebutton-plugin.php', apply_filters( 'active_plugins',$active_plugins ) )){
    		add_action('admin_notices','wplms_emails_migrate_notice');
    	}
    },2);
    ...
    function wplms_emails_migrate_notice(){
    	$class = 'notice notice-error is-dismissible';
            ...
            $message = sprintf(__( 'Wplms bbb needs %s plugin to be activated.', 'wplms-bbb' ),'<a href="https://wordpress.org/plugins/bigbluebutton/">Bigbluebutton</a>');
	...
	function wplms_bbb_translations(){
	    ...
	    $mofile        = sprintf( '%1$s-%2$s.mo', 'wplms-bbb', $locale );
